  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 sm:px-6 py-3 sm:py-4 bg-surface-container-low/50 border-b border-outline/10 flex flex-wrap items-center gap-2 sm:gap-3 shrink-0">
        ...
        <h3 className="text-[0.75rem] font-bold text-on-surface-variant uppercase tracking-[0.2em] break-words">Today's Alerts</h3>
      ...
      <div className="divide-y divide-outline/5 overflow-y-auto">
        {notifications.length === 0 ? (
          <div className="p-10 text-center opacity-20">
            <p className="text-[0.85rem] font-medium italic">No notifications for today</p>
          </div>
        ) : (
          notifications.map((n) => (
            <div 
            ...
            >
              ...
              <div className="flex items-center gap-3 shrink-0 w-full sm:w-auto">
                ...
                <h4 className={`sm:hidden flex-1 text-[0.85rem] ${!n.read ? 'font-bold' : 'font-semibold'} text-on-surface group-hover:text-primary-container transition-colors tracking-tight truncate`}>
                  {n.title}
                </h4>
              </div>
              
              <div className="flex-1 min-w-0 flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-8 pl-11 sm:pl-0">
                <h4 className={`hidden sm:block w-[200px] shrink-0 text-[0.85rem] ${!n.read ? 'font-bold' : 'font-semibold'} text-on-surface group-hover:text-primary-container transition-colors tracking-tight truncate`}>
                  {n.title}
                </h4>
              ...
              <div className="shrink-0 w-full sm:w-auto sm:min-w-[150px] text-right relative flex items-center justify-end pl-11 sm:pl-0 mt-2 sm:mt-0">
                <span className="text-[0.7rem] text-on-surface-variant/40 font-bold uppercase sm:group-hover:opacity-0 transition-all hidden sm:block">{n.time}</span>
                <div className="sm:absolute sm:right-0 flex gap-2 w-full sm:w-auto sm:opacity-0 sm:group-hover:opacity-100 transition-all">
                  ...
                  <button 
                    ...
                    className="px-3 py-1.5 sm:py-1 bg-error/10 text-error rounded-sm text-[0.75rem] sm:text-[0.7rem] font-bold hover:bg-error/20"
                  >
                    Clear
                  </button>
                </div>
