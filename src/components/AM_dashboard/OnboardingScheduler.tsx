...
interface Event {
  id: string;
  title: string;
  category: 'Internal Meetings' | 'Client AM' | 'Training Session';
...
const AMScheduler: React.FC = () => {
  ...
  const handleCreateEvent = (eventData: any) => {
    const colorMap: Record<string, string> = {
      ...
      'Client AM': 'rgb(var(--color-primary-container))',
  ...
  return (
    <div className="flex h-full w-full bg-surface font-body selection:bg-primary-container/30 overflow-hidden animate-in fade-in duration-300">
      {/* Sidebar */}
      <aside className="w-[280px] border-r border-outline/10 flex flex-col bg-surface-container-low shrink-0 h-full">
                  ...
                  <div className="flex-1 p-4">
                    <div className="flex items-center justify-between mb-2">
                      <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-sm ${isDark ? 'bg-surface-container-high/50' : 'bg-surface-container-low'} text-[0.6rem] font-bold uppercase tracking-wider border border-outline/5`} style={{ color: event.color }}>
                        <span className="material-symbols-outlined !text-[12px]">{event.category === 'Client AM' ? 'rocket_launch' : event.category === 'Training Session' ? 'school' : 'groups'}</span>
                        {event.category}
                      </div>
      ...
      <main className="flex-1 flex flex-col bg-surface overflow-hidden relative">
        <header className="px-8 py-4 flex justify-between items-center bg-surface border-b border-outline/10 shrink-0">
          <div>
            <h1 className="text-[1.5rem] font-bold text-on-surface tracking-tighter uppercase font-headline">
