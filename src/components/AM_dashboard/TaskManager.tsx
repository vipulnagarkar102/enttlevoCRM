...
const TaskManager: React.FC<TaskManagerProps> = ({ setIsAdding, setEditingTask, refreshTrigger }) => {
  ...
  const [tasks, setTasks] = useState<Task[]>([
    { id: '1', title: 'System Configuration', category: 'Configuration', date: 'Mar 25, 2026', time: '10:00', user: 'rushib', isCompleted: false },
    { id: '2', title: 'User Training Session', category: 'Training', description: 'Webinar for the sales team', date: 'Mar 26, 2026', time: '14:00', user: 'rushib', isCompleted: false },
    { id: '3', title: 'Initial Kick-off', category: 'Meeting', description: 'Project Scope', date: 'Mar 20, 2026', time: '11:00', user: 'rushib', isCompleted: true, completedAt: 'Mar 20, 2026' },
  ]);

  useEffect(() => {
    if (refreshTrigger && refreshTrigger > 0) {
      // In a real app, this would trigger a refetch or the overlay would call a callback with the new task
    }
  }, [refreshTrigger]);
    ...
    <div className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 bg-surface selection:bg-primary-container/30 relative animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-2">
        <div className="text-left">
          <h1 className="text-[1.75rem] font-medium tracking-tight text-on-surface leading-tight font-headline uppercase">AM Task Manager</h1>
          <p className="text-on-surface-variant text-[0.9rem] mt-0.5 font-body">Track implementation and training tasks</p>
        </div>
      ...
      {/* Content Columns */}
      <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 items-stretch lg:items-start pt-2">
        {/* Active Column (Pending) */}
        <div className="w-full lg:flex-[0.65] space-y-4">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-sm ${isDark ? 'bg-indigo-900/30 text-indigo-300' : 'bg-indigo-50 text-indigo-600'} text-[0.65rem] font-bold uppercase tracking-wider border border-outline/5`}>
                        <span className="material-symbols-outlined !text-[14px]">{task.category === 'Meeting' ? 'groups' : task.category === 'Configuration' ? 'settings' : 'task'}</span>
                        {task.category}
                      </div>
                    </div>
        ...
        {/* Completed Column */}
        <div className="w-full lg:flex-[0.35] space-y-4">
