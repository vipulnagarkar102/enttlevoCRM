...
interface Event {
  id: string;
  title: string;
  category: 'Internal Strategy' | 'Client Meeting' | 'Account Review' | 'Renewal Sync';
...
const AMScheduler: React.FC = () => {
  ...
  const [events, setEvents] = useState<Event[]>([
    ...
    {
      id: '3',
      title: 'Mirrat Renewal Discussion',
      category: 'Renewal Sync',
      date: new Date().toISOString().split('T')[0],
      startTime: '04:30 PM',
      endTime: '05:30 PM',
  ...
  const handleCreateEvent = (eventData: any) => {
    const colorMap: Record<string, string> = {
      'Internal Strategy': '#3B82F6',
      'Client Meeting': 'rgb(var(--color-primary-container))',
      'Account Review': '#F59E0B',
      'Renewal Sync': '#10B981',
    };
    ...
    <div className="flex h-full w-full bg-surface font-body selection:bg-primary-container/30 overflow-hidden animate-in fade-in duration-300 relative">
        ...
        <div className="flex-1 p-5 space-y-8 custom-scrollbar">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-7 gap-y-1 text-center">
              ...
              {trailingDays.map(d => (
                <span key={`prev-${d}`} className="text-[0.7rem] py-1 opacity-20 text-on-surface-variant">{d}</span>
              ))}
              {days.map(d => (
                <button 
                   key={d} 
          ...
          <div className="space-y-4 pt-4 border-t border-outline/10 flex-1 overflow-y-auto custom-scrollbar-mini pr-2">
            <h3 className="text-[0.65rem] font-bold text-on-surface-variant/60 uppercase tracking-[0.15em] sticky top-0 bg-surface-container-low z-10 py-2">Meetings for {new Date(selectedDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</h3>
            <div className="space-y-4 relative">
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-sm ${isDark ? 'bg-surface-container-high/50' : 'bg-surface-container-low'} text-[0.6rem] font-bold uppercase tracking-wider border border-outline/5`} style={{ color: event.color }}>
                        <span className="material-symbols-outlined !text-[12px]">
                          {event.category === 'Client Meeting' ? 'handshake' : event.category === 'Account Review' ? 'analytics' : event.category === 'Renewal Sync' ? 'autorenew' : 'psychology'}
                        </span>
                        {event.category}
                      </div>
              ...
              {events.filter(e => e.date === selectedDate).length === 0 && (
                <div className="py-10 text-center opacity-40 flex flex-col items-center gap-2">
                   ...
                   <p className="text-[0.75rem] font-medium">No plans for today</p>
                </div>
              )}
            </div>
          </div>
      ...
      <main className="flex-1 flex flex-col bg-surface overflow-hidden relative min-w-0">
        ...
        <div className="flex-1 overflow-x-auto overflow-y-hidden custom-scrollbar">
          ...
          {(view === 'Month' ? days : days.slice(Math.max(0, currentDate.getDate() - 3), currentDate.getDate() + 4)).map(d => {
        ...
        {/* Toast Notifs */}
        {toast.visible && (
            <div className="fixed bottom-10 left-1/2 -translate-x-1/2 bg-inverse-surface text-inverse-on-surface px-8 py-4 rounded-sm shadow-2xl flex items-center gap-4 animate-in slide-in-from-bottom-5 duration-500 z-[2000]">
