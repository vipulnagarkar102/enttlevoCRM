...
type TaskCategory = 'Meeting' | 'Follow up' | 'Mail' | 'Message' | 'Call' | 'Configuration' | 'Training';
...
const TasksTabContent = () => {
  ...
  const getCategoryStyles = (category: TaskCategory) => {
    switch (category) {
      ...
      case 'Configuration': return { bg: isDark ? 'bg-blue-900/30' : 'bg-blue-500/10', text: isDark ? 'text-blue-300' : 'text-blue-600', icon: 'settings' };
      case 'Training': return { bg: isDark ? 'bg-rose-900/30' : 'bg-rose-500/10', text: isDark ? 'text-rose-300' : 'text-rose-600', icon: 'school' };
    ...
    <div className="flex flex-col h-full w-full bg-surface animate-in fade-in duration-300">
      ...
      <div className="p-6 border-b border-outline/10 flex justify-between items-center bg-surface z-10 sticky top-0">
        <div>
          ...
          <p className="text-[0.8rem] text-on-surface-variant font-medium">Manage implementation tasks</p>
        </div>
        <button 
