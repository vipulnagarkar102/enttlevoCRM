...
const SankeyChart: React.FC<SankeyChartProps> = ({ className }) => {
  ...
  return (
    <div className={`${className || 'col-span-8'} p-6 bg-surface-container-low rounded-sm border border-outline/5 h-[400px] transition-colors duration-300`}>
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-4">
          <div className="flex items-center gap-2 text-[0.7rem] text-on-surface-variant font-medium bg-surface-container px-2 py-1 rounded-sm border border-outline/5">
            Success Rate: <span className="text-primary-container">84.2%</span>
          </div>
        </div>
      </div>
      <div className="h-[280px]">
        <ResponsiveContainer width="100%" height="100%">
          <Sankey
            data={{
              nodes: [
                { name: 'Onboarding Initiated', color: '#ffb787' },
                { name: 'Initial Setup', color: '#3b82f6' },
                { name: 'Documentation Review', color: '#f87171' },
                { name: 'Training Sessions', color: '#fb923c' },
                { name: 'Configuration', color: '#60a5fa' },
                { name: 'User Acceptance', color: '#10b981' },
                { name: 'Go-Live Review', color: '#f59e0b' },
                { name: 'Fully Onboarded', color: '#059669' },
                { name: 'Stalled/Inactive', color: '#ef4444' },
              ],
              links: [
                ...
                { source: 5, target: 6, value: 78 },
                { source: 6, target: 7, value: 75 },
